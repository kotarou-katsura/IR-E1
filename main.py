...
def colorizePrint(color_name, message):
    """Print a message using a named color/style from Colors."""
...
for idx, pdf_path in enumerate(pdf_files, 1):
    ...
    colorizePrint("BLUE", "List of lemmatized words (terms):")
    lemma_list = sorted(list(set(lemma_terms)), key=str.lower)
    print("  ", lemma_list[:100], "...")
    print()
...
for doc_id, lemmas in enumerate(lemma_sets):
    lowered = [t.lower() for t in lemmas]
...
def _tokenize_boolean_query(raw_query: str):
    ...
    for p in parts:
        ...
        if up in ("AND", "OR", "NOT", "(", ")"):
            ...
            # normalize term to lowercase, keep alphas
            doc = nlp(p)
            term_tokens = [t.text.lower() for t in doc if t.is_alpha]
            if term_tokens:
...
def _doc_terms_list(doc_id):
    return [t.lower() for t in lemma_sets[doc_id]]

def _doc_text(doc_id):
    return raw_texts[doc_id]

def _find_phrase_in_doc(phrase, doc_id):
    terms = [t.lower() for t in tokenize(phrase) if t.is_alpha]
    if not terms:
        return False
...
def _same_sentence(term1, term2, doc_id):
    ...
    for sent in doc_spacy.sents:
        sent_terms = [t.text.lower() for t in sent if t.is_alpha]
...
def _same_paragraph(term1, term2, doc_id):
    ...
    for p in paras:
        pdoc = tokenize(p)
        pterms = [t.text.lower() for t in pdoc if t.is_alpha]
        if term1 in pterms and term2 in pterms:
            return True
    return False

def _wildcard_postings(pattern):
    # convert '!' to '.*' and match against indexed terms
...
def extended_boolean_model(raw_query):
    ...
    while i < len(tokens):
        ...
        if tok.startswith('"'):
            ...
            term2 = tokens[i + 2]
            t1 = next((t.text.lower() for t in tokenize(term1) if t.is_alpha), None)
            t2 = next((t.text.lower() for t in tokenize(term2) if t.is_alpha), None)
            if t1 and t2:
            ...
            if i + 1 < len(tokens):
                t2_raw = tokens[i + 1]
                t1 = next((t.text.lower() for t in tokenize(t1_raw) if t.is_alpha), None)
                t2 = next((t.text.lower() for t in tokenize(t2_raw) if t.is_alpha), None)
                if t1 and t2:
                    if mode == '/s':
            ...
            t = next((t.text.lower() for t in tokenize(tok) if t.is_alpha), None)
            if t:
...
def show_standard_boolean_help():
    ...
    colorizePrint("CYAN", "  Terms are normalized (lowercased).")
